...
from common import overunder_flowbin, get_mcEventnumber, all_trigger, for_diele_trigger, for_singleele_trigger_eechannel, kinematic, select_MC_event, for_singlemuon_trigger_mumuchannel, for_dimuon_trigger
import numpy as np
import optparse
...
def analysis(era, isMC, add_trigger_SF, fin, xs, cfsf, shift, start, end, label):
  if(era == '2016'):
  ...
  TTC_header_path = os.path.join("scripts/TTC_" + era + "CoD.h")
...
#  SS_filters="ttc_region==3 && ttc_mll>60 && ttc_mll<120 && (ttc_l1_pt>30 || ttc_l2_pt>30) && ttc_drll>0.3"
  if isMC:
    #OS_filters="OPS_region==3 && OPS_z_mass>%f && OPS_z_mass<%f && MET_T1Smear_pt < 50 && n_tight_jet < 3 && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_BadPFMuonFilter && OPS_l1_pt>20 && OPS_l2_pt>20 && abs(OPS_l1_eta)<2.4 && abs(OPS_l2_eta)<2.4"%((OS_Zmass-OS_width),(OS_Zmass+OS_width)) 
    OS_filters="OPS_region==1 && OPS_2P0F && OPS_z_mass>60 && OPS_z_mass<120 && OPS_l1_pt>30 && OPS_l2_pt>20 && OPS_drll>0.3 && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_BadPFMuonFilter && Flag_eeBadScFilter && Flag_ecalBadCalibFilter && nHad_tau==0"
    SS_filters="ttc_region==1 && ttc_mll>%f && ttc_mll<%f && MET_T1Smear_pt < 50 && n_tight_jet <3 && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_BadPFMuonFilter && ttc_l1_pt>20 && ttc_l2_pt>20 && abs(ttc_l1_eta)<2.4 && abs(ttc_l2_eta)<2.4"%((SS_Zmass-SS_width),(SS_Zmass+SS_width))
  else:
    #OS_filters="OPS_region==3 && OPS_z_mass>%f && OPS_z_mass<%f && MET_T1_pt < 50 && n_tight_jet < 3 && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_BadPFMuonFilter && OPS_l1_pt>20 && OPS_l2_pt>20 && abs(OPS_l1_eta)<2.4 && abs(OPS_l2_eta)"%((OS_Zmass-OS_width),(OS_Zmass+OS_width))
    OS_filters="OPS_region==1 && OPS_2P0F && OPS_z_mass>60 && OPS_z_mass<120 && OPS_l1_pt>30 && OPS_l2_pt>20 && OPS_drll>0.3 && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_BadPFMuonFilter && Flag_eeBadScFilter && Flag_ecalBadCalibFilter && nHad_tau==0"
    SS_filters="ttc_region==1 && ttc_mll>%f && ttc_mll<%f && MET_T1_pt < 50 && n_tight_jet <3 && Flag_HBHENoiseFilter && Flag_HBHENoiseIsoFilter && Flag_EcalDeadCellTriggerPrimitiveFilter && Flag_goodVertices && Flag_globalSuperTightHalo2016Filter && Flag_BadPFMuonFilter && ttc_l1_pt>20 && ttc_l2_pt>20 && abs(ttc_l1_eta)<2.4 && abs(ttc_l2_eta)<2.4"%((SS_Zmass-SS_width),(SS_Zmass+SS_width))
  ...
  if isMC:
    ...
    if "DoubleMuon" in fin:
      df_OS_tree = for_dimuon_trigger(df_OS_tree,era)
      df_SS_tree = for_dimuon_trigger(df_SS_tree,era)
    elif "SingleMuon" in fin:
      df_OS_tree = for_singlemuon_trigger_mumuchannel(df_OS_tree,era)
      df_SS_tree = for_singlemuon_trigger_mumuchannel(df_SS_tree,era)
  ...
  if shift==1:
    ...
    shift_name = "_DOWN/"
  fout = ROOT.TFile.Open("flatten/era" + era + "/" + cfsf_name + shift_name + "h" + str(label) + "_" + fin, "RECREATE")
  fout.cd()
...
if __name__ == "__main__":
  ...
  parser.add_option('-c','--cfsf',dest='cfsf',help='Add chargeflip SF or not', default =0, type=int)
  ...
  analysis(args.era, args.isMC, args.trig, args.fin, args.xsec, args.cfsf, args.shift, args.start, args.end, args.label)
