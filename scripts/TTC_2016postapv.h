...
TH2F*h_SS  =(TH2F*)f_cf->Get("SS_ChargeFlip_SF");
...
TFile*fmu =TFile::Open("data/muonIdSF_" + era + "UL.root");
TH2D*h_muSF =(TH2D*)fmu->Get("muIdSF_2016");
TH2F*h_RECO =(TH2F*)fmu->Get("NUM_TrackerMuons_DEN_genTracks");
...
float muID_sf_mumu(float l1_pt, float l2_pt, float l1_eta, float l2_eta){
        if(l1_pt>120) l1_pt=119.;
        if(l2_pt>120) l2_pt=119.;
        float sf_l1=h_muSF->GetBinContent(h_muSF->FindBin(fabs(l1_eta),l1_pt));
        float sf_l2=h_muSF->GetBinContent(h_muSF->FindBin(fabs(l2_eta),l2_pt));
        if(l1_pt>40) l1_pt=39;
        if(l2_pt>40) l2_pt=39;
        float sf_reco1=h_RECO->GetBinContent(h_RECO->FindBin(fabs(l1_eta),l1_pt));
        float sf_reco2=h_RECO->GetBinContent(h_RECO->FindBin(fabs(l2_eta),l2_pt));
        return sf_l1*sf_l2*sf_reco1*sf_reco2;
}
