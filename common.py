...
def for_singlemuon_trigger_mumuchannel(df,era):
  if '2016' in era:
...
def select_MC_event(df_MC_tree, add_trigger_SF, add_chargeflip_SF, shift, filters, isOS, era, process):
  ...
  if isOS:
    ...
    df_MC_tree = df_MC_tree.Define("trigger_SF", "trigger_sf_mm(OPS_l1_pt, OPS_l2_pt, OPS_l1_eta, OPS_l2_eta)")
    df_MC_tree = df_MC_tree.Define("muID_SF", "muID_sf_mumu(OPS_l1_pt, OPS_l2_pt, OPS_l1_eta, OPS_l2_eta)")
 #   df_MC_tree = df_MC_tree.Define("muID_SF","1.")
    ...
    df_MC_tree = df_MC_tree.Define("trigger_SF", "trigger_sf_mm(ttc_l1_pt, ttc_l2_pt, ttc_l1_eta, ttc_l2_eta)")
    df_MC_tree = df_MC_tree.Define("ttc_kinematic_region","kinematic(ttc_l1_pt, ttc_l2_pt, ttc_l1_eta, ttc_l2_eta)")
    df_MC_tree = df_MC_tree.Define("muID_SF", "muID_sf_mumu(ttc_l1_pt, ttc_l2_pt, ttc_l2_eta, ttc_l2_eta)")
    if(add_chargeflip_SF and (('DY' in process) or ('TTTo' in process))):
  ...
  if not add_trigger_SF:
    ...
    if isOS:
      df_MC_tree = df_MC_tree.Define("genweight","puWeight*PrefireWeight*muID_SF*charge_flip_SF*genWeight/abs(genWeight)")
    else:
      df_MC_tree = df_MC_tree.Define("genweight","puWeight*PrefireWeight*muID_SF*charge_flip_SF*genWeight/abs(genWeight)")
    ...
    if isOS:
      df_MC_tree = df_MC_tree.Define("genweight","puWeight*PrefireWeight*muID_SF*trigger_SF*charge_flip_SF*genWeight/abs(genWeight)")
    else:
      df_MC_tree = df_MC_tree.Define("genweight","puWeight*PrefireWeight*muID_SF*trigger_SF*charge_flip_SF*genWeight/abs(genWeight)")
  df_MC = df_MC_tree.Filter(filters)

  return all_trigger(df_MC,era)
